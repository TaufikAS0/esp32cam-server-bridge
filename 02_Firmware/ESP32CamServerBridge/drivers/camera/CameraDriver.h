#ifndef DRIVERS_CAMERA_CAMERA_DRIVER_H
#define DRIVERS_CAMERA_CAMERA_DRIVER_H
...
class CameraDriver {
 public:
  bool begin(String* errorMessage) {
    ...
    if (result != ESP_OK) {
      if (errorMessage != nullptr) {
        *errorMessage =
            "esp_camera_init failed with code " + String(static_cast<int>(result));
      }
      return false;
    }
